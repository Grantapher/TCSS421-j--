...
class JMessageExpression extends JExpression {
    ...
    protected JMessageExpression(int line, JExpression target, String messageName,
    ...
    public JExpression analyze(Context context) {
        ...
        if (target == null) {
            ...
            if (!context.methodContext().isStatic()) {
                ...
                target = new JVariable(line(), context.definingType().toString())
                        .analyze(context);
            }
        } else {
        ...
        if (method == null) {
            JAST.compilationUnit.reportSemanticError(line(), "Cannot find method for: " +
                                                             Type.signatureFor(
                                                                     messageName, argTypes
                                                             )
            );
            type = Type.ANY;
        } else {
    ...
    public void codegen(CLEmitter output) {
        ...
        int mnemonic = method.isStatic() ? INVOKESTATIC
                : target.type().isInterface() ? INVOKEINTERFACE : INVOKEVIRTUAL;
