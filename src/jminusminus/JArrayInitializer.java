...
class JArrayInitializer extends JExpression {
    ...
    public JArrayInitializer(int line, Type expected, ArrayList<JExpression> initials) {
        super(line);
        type = expected;
    ...
    public JExpression analyze(Context context) {
        ...
        if (!type.isArray()) {
            JAST.compilationUnit.reportSemanticError(line, "Cannot initialize a " +
                                                             type.toString() +
    ...
    public void codegen(CLEmitter output) {
        ...
        for (int i = 0; i < initials.size(); i++) {
            ...
            } else if (componentType == Type.FLOAT) {
                output.addNoArgInstruction(FASTORE);
