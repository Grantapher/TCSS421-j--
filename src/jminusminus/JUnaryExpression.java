...
class JNegateOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        ...
        arg.type().mustMatchOneOf(line(), Type.INT, Type.FLOAT, Type.LONG, Type.DOUBLE);
        type = arg.type();
        return this;
    }
    ...
    public void codegen(CLEmitter output) {
        ...
        if (type().equals(Type.INT)) {
            ...
            output.addNoArgInstruction(FNEG);
        } else { //double
...
class JPostDecrementOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        if (!(arg instanceof JLhs)) {
            JAST.compilationUnit
                    .reportSemanticError(line, "Operand to expr-- must have an LValue.");
            type = Type.ANY;
        } else {
            arg = (JExpression) arg.analyze(context);
            arg.type().mustMatchOneOf(line(), Type.INT, Type.LONG, Type.FLOAT, Type.DOUBLE);
            type = arg.type();
        }
        return this;
    }
    ...
    public void codegen(CLEmitter output) {
        if (arg.type() == Type.INT && arg instanceof JVariable) {
